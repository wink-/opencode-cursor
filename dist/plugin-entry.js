...
var init_plugin = __esm(() => {
  ...
  AUTO_TRUST_WORKSPACE = process.env.CURSOR_ACP_TRUST !== "false";
...
init_executor();
init_plugin();
var log24 = createLogger("plugin-v2");
function routeRequestToProxy(request, baseURL) {
...
function v2ToolFromV1(name, entry, jsonSchema) {
  const description = typeof entry?.description === "string" ? entry.description : name;
  const input = jsonSchema && typeof jsonSchema === "object" ? jsonSchema : { type: "object", properties: {} };
...
function createV2Setup() {
  return async (ctx) => {
    ...
    if (!state.enabled) {
      log24.info("Plugin disabled in OpenCode config; skipping initialization", {
    ...
    log24.debug("V2 plugin initializing", {
      workspaceDirectory
    });
    await ensurePluginDirectory();
    autoRefreshModels().catch(() => {});
    ...
    if (mcpEnabled) {
      try {
        ...
        log24.debug("MCP bridge init failed", { error: String(err) });
      }
    }
    const toolsEnabled = process.env.CURSOR_ACP_ENABLE_OPENCODE_TOOLS !== "false";
    const legacyProxyToolPathsEnabled = toolsEnabled && TOOL_LOOP_MODE === "proxy-exec";
    ...
    const localTools = buildLocalFallbackTools(localRegistry, TOOL_LOOP_MODE);
    for (const tool3 of localTools)
      toolsByName.set(tool3.name, tool3);
    skillResolver = new SkillResolver(skillLoader.load(localTools));
    const lastToolNames = localTools.map((tool3) => tool3.name);
    const lastToolMap = localTools.map((tool3) => ({ id: tool3.id, name: tool3.name }));
    const proxyBaseURL = await ensureCursorProxyServer(workspaceDirectory, router ?? undefined);
    log24.debug("Proxy server started", { baseURL: proxyBaseURL });
    registrations.push(await ctx.session.hook("http.request", async (event) => {
      if (event.model.providerID !== CURSOR_PROVIDER_ID2)
        return;
    ...
    registrations.push(await catalog.transform((catalogDraft) => {
      catalogDraft.provider.update(CURSOR_PROVIDER_ID2, (p) => {
        p.name = "Cursor";
    ...
    registrations.push(await ctx.integration.transform((integrations) => {
      integrations.method.update({
        integrationID: CURSOR_PROVIDER_ID2,
    ...
    try {
      ...
      log24.debug("Tool registration failed", { error: String(err) });
    }
    registrations.push(await ctx.session.hook("context", async (event) => {
      const modelRef = event.model;
      const isCursor = modelRef?.providerID === CURSOR_PROVIDER_ID2;
      if (!isCursor)
        return;
      try {
        const connection = await ctx.integration.connection.active(CURSOR_PROVIDER_ID2);
        ...
        log24.debug("Could not resolve Cursor API key", { error: String(err) });
      }
      const systemMessage = buildAvailableToolsSystemMessage(lastToolNames, lastToolMap, mcpToolDefs, mcpToolSummaries);
      if (systemMessage) {
...
// src/plugin-entry.ts
var log25 = createLogger("plugin-entry");
var CursorPluginEntry = async (input) => {
  const state = shouldEnableCursorPlugin();
  if (!state.enabled) {
    log25.info("Plugin disabled in OpenCode config; skipping initialization", {
      configPath: state.configPath,
      reason: state.reason
    });
    return {};
